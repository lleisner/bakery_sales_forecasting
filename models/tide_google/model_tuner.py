...
def build_tide(hp, learning_rate, seq_len, pred_len, num_ts, cat_sizes=[]):
    hidden_size = hp.Choice('hidden_size', values=[2**i for i in range(3, 10, 2)])  # 8, 32, 128, 512
    decoder_output_dim = hp.Choice('decoder_output_dim', values=[2**i for i in range(3, 8)])  # 8, 16, 32, 64, 128
    final_decoder_hidden = hp.Choice('final_decoder_hidden', values=[2**i for i in range(3, 7)])  # 8, 16, 32, 64
