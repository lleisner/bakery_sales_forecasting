    ...
    def load_encoder(self) -> StandardScaler:
        if self.encoder_filename:
            try:
                ...
                raise Exception("Error while loading encoder. Try running DataEncoder.fit_encoder() or DataEncoder.fit_and_encode() first to create a new encoder") from e
...
class TemporalEncoder(DataEncoder):
...
class WeatherEncoder(DataEncoder):
