...
class DataProcessor:
    ...
    def _create_sales_features(self) -> None:
        self._clean_labels()
        sales_feature = self.column_groups['labels'].copy().add_prefix(f'(t-{self.future_days})')
        sales_feature.index = sales_feature.index + pd.Timedelta(days=self.future_days)
        self.column_groups['sales'] = sales_feature
        
    def _create_sales_features(self):
        self._clean_labels()
    ...
    def _clean_labels(self):
        sales_data = self.column_groups['labels']
        sales_data = sales_data.mask(sales_data < 0, 0)

        # Define the valid time range
        valid_time_range = pd.to_datetime(sales_data.index).to_series().between_time('8:00', '17:00')

        # Create a mask for rows outside the valid time range
        invalid_rows_mask = ~sales_data.index.isin(valid_time_range.index)

        # Set values in rows outside the valid time range to 0
        sales_data.loc[invalid_rows_mask] = 0

        # Calculate row sums
        row_sums = sales_data.sum(axis=1)

        # Shift the data to get adjacent row sums
        prev_row_sums = row_sums.shift(1)
        next_row_sums = row_sums.shift(-1)

        # Set rows to 0 if the sum is much smaller than adjacent rows
        threshold = 0.15
        outlier_rows = (row_sums < prev_row_sums * threshold) | (row_sums < next_row_sums * threshold)

        # Set outlier rows to 0
        sales_data.loc[outlier_rows] = 0

        self.column_groups['labels'] = sales_data
        
        # Create 'is_open' column based on sales values in self.column_groups
        self.column_groups['is_open'] = sales_data.any().astype(int)


    def get_uncoded_data(self):
        datetime_df = pd.DataFrame({'datetime': self.column_groups['datetime']})

        # Extracting DataFrames for concatenation
        #data_frames = [datetime_df] + [self.column_groups[key] for key in ['weather', 'ferien', 'fahrten', 'labels', 'sales']]
        data_frames = [datetime_df] + [self.column_groups[key] for key in self.column_groups.keys() if key != 'datetime']
    ...
    def decode_data(self, data: pd.DataFrame):
        ...
        return pd.DataFrame(decoder.decode_data(data), columns=self.column_groups['labels'].columns, index=data.index).astype(int)
    
    def get_shape(self):
        num_features = sum(len(value.columns) for key, value in self.column_groups.items() if key not in ['datetime', 'labels'])
        num_targets = len(self.column_groups['labels'].columns)
