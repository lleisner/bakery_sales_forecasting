...
def custom_time_series_loss(future_steps, length_of_day):
    ...
    def loss_function(y_true, y_pred):
        # Slice the last n elements of the sequences
        y_true_last_n = y_true[:, -future_steps:]
        y_pred_last_n = y_pred[:, -future_steps:]

        # Calculate the sum of values for entire days in the last n steps
        y_true_day_sum = tf.reduce_sum(tf.reshape(y_true_last_n, (-1, length_of_day)), axis=1)
        y_pred_day_sum = tf.reduce_sum(tf.reshape(y_pred_last_n, (-1, length_of_day)), axis=1)

        # Compute the mean squared error for the last n elements with weights
        mse = tf.reduce_mean(tf.square(y_true_last_n - y_pred_last_n))
